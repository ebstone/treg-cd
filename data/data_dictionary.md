...
| `cms_asp_and_hcup_cost_sources.csv` | CMS ASP drug unit costs, CMS Physician Fee Schedule administration cost, HCUP colectomy surgery cost (provenance only, not used in the base case — see below), FSS unit costs | CMS, HCUPnet, Aliyev et al. 2019 Table 2 | URLs are general program pages, not deep links to the specific pricing file used (the workbook cites monthly pricing files by name but the underlying files were not included in the project). |
