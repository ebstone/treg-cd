...
| **1. Foundations** | 2–3 | Set up repository, `renv`, testing harness. Re-derive all transition probabilities from published endpoints via DEALE (§7.3) and diff against v6. Resolve and document the inflation index (§8). Re-extract comparator ASP at a stated date | Model lead (Jadambaa) | **Gate 1: re-derived probabilities reconcile with v6, or discrepancies are explained in writing** — **scope narrowed 2026-08-04 (E. Stone):** reconciliation with the legacy v6/manuscript figures is no longer the pass condition. Those figures carry audited defects (`docs/model_audit_v6.md`) and are retained only as reference context; Gate 1 now requires each re-derived probability to rest on a single documented, defensible method with no unresolved candidate ambiguity, which was achieved for UST/ADA induction and the UST/CT maintenance Remission cell. IFX induction (no direct trial endpoint) and the IFX/ADA maintenance matrix remain open data gaps. See `R/00_derive_transition_probs.R` and `data/processed/DERIVATION_NOTES.md`. |
