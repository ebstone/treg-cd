...
*Answerable as of 2026-08-06 only* — until the PSA sampled the relapse hazard (B3, `docs/model_audit_v6.md` A19), the durability block did not exist as a distinct subset. First result, on a 300-draw lifetime-horizon check pending the authoritative 10,000-draw run: **it does** — A∪B ≈ 93% of total EVPI against ≈10% for the cost block, and the relapse hazard alone outranks acquisition price. §9.3 has the subset table and its current status.
...
| 10 | Post-cure annual relapse hazard h | See Section 7.2 | **Gamma**, mean ln 2 / 10 = 0.0693/yr, shape 2 (this row previously said "Log-normal, wide") | **Assumption — parameterised and sampled 2026-08-06** (B3, `docs/model_audit_v6.md` A19). Gamma, not Log-normal: h is a *rate*, and Gamma is the standard PSA distribution for a rate (Briggs, Claxton & Sculpher 2006 §4.3) — conjugate to the Poisson process a relapse count actually is, and used elsewhere in this same table for the cost rows for the same reason. The mean is fixed at the §7.2 base-case anchor of a 10-year median SDR duration, so the PSA's centre and the deterministic default are the same number by construction; shape 2 gives a median implied duration of 11.9 years and an IQR of 7.4–20.8 years. `R/06_psa.R`'s `sample_relapse_hazard_draws()` |
...
**Amended 2026-08-06 (peer review's B3, `docs/model_audit_v6.md` A19) — the correction above was written but not wired in.** The 2026-08-05 status note says the base case is T = 10 years. It was not: `headroom_pi_star()`/`headroom_frontier()` (R/05) and `ejp_deterministic()`/`ejp_frontier()` (R/08) all still *defaulted* h = 0, and `analysis/run_full_analysis.R` passes none of them explicitly, so every π\* and EJP figure published up to and including 2026-08-06's post-B1 run was computed at h = 0 — the permanent-cure assumption this very paragraph identifies as the most favourable available, not the T = 10 base case it claims. `R/06_psa.R` did the same, hardcoding h = 0 on all 10,000 draws. All five defaults now use `duration_to_hazard(DEFAULT_RELAPSE_DURATION_YEARS)`, and the PSA samples h around that same mean (item 10 in §7.1's table above). **This distinction matters more than the T sweep alone suggested it would**: at the lifetime horizon, moving from h = 0 to T = 10 years raises the required cure fraction π\* at Treg's sourced price from 16.8% to 44.6% at WTP $150k. A "base case" recorded in a constant but read by nothing is not a base case, and this is the second time this project has shipped that specific failure mode (A16 was the first, for the Remission utility) — both are now guarded by tests that check a default against its source rather than against itself.
...
**Status of the subset table, 2026-08-06 (B3, `docs/model_audit_v6.md` A19).** Subsets **A, B, A∪B, C and E are implemented and computed**; D, F and G remain undefined, because EVPPI over a parameter the PSA holds fixed does not exist (A14) and `R/06_psa.R` still does not sample comparator maintenance transitions, induction response probabilities, or the surgery pathway — no sourced concentration parameter exists for any of them. B and A∪B were the ones added on this date: until `run_psa()` sampled the relapse hazard, subset B was undefined and A∪B was just A wearing the durability block's label, which meant **Aim 3's falsifiable claim could not be evaluated at all**. `evppi_by_subset()`'s all-parameters row is correspondingly now A∪B∪C∪E. Note when reading the table that A∪B is 2 raw dimensions and so is *not* PCA-reduced, unlike any union involving E — the durability block is therefore free of the dilution artifact described below, which is a reason to lead Aim 3 with it rather than with the wider unions.

**Sample size.** Use **10,000 PSA draws minimum**, not the current 1,000. EVPPI estimates from 1,000 draws are unstable, particularly for regression-based estimators, and a reviewer familiar with the method will ask. Report a convergence check (EVPPI estimate vs. number of draws) — run for subset A *and* subset B separately (`evppi_convergence_subset_A.csv` / `_B.csv`), since a converged A says nothing about whether the much smaller B is a resolved number, and Aim 3's comparison depends on B being real rather than noise.
...
| **Constant relapse hazard from SDR** | Oncology mixture-cure models often use declining hazards | Acknowledged; a constant hazard is the simplest defensible shape, **not a conservative one** — corrected 2026-08-05, see §15 Decision 4's status note (the row previously claimed "conservative over a lifetime horizon", which is the framing error that memo exists to fix: a *declining* hazard would be more favourable to Treg than a constant one, and h = 0 most favourable of all). The hazard's magnitude is anchored at a 10-year median SDR duration in the base case, swept over T ∈ {2, 5, 10, 20, ∞} deterministically, and sampled from a Gamma prior in the PSA (§7.1 item 10, 2026-08-06) |
...
| **No adverse events modelled for a cell therapy (peer review 2026-08-05, M5)** | Allogeneic cell products carry immunogenicity, infusion-reaction and (where preconditioning is used) cytopenia risks; this project's own cyclophosphamide preconditioning dose is *also* unpriced (`cyclophosphamide_dose_mg` defaults to 0, `R/04_costs_utilities.R`), so the gap compounds on Treg's own conditioning regimen specifically, not just a generic omission shared evenly across arms | **Currently a real gap, deliberately deferred, not overlooked — confirmed 2026-08-06, Eric's own explicit decision to leave out of that day's work and keep as a next step.** At minimum, model administration-related costs including any preconditioning and observation stay, and run a scenario with a disutility and cost for serious adverse events. Reviewers of a cell-therapy CEA will expect this, and of the four items in this deferral, this is the one most likely to draw a direct reviewer question — recommend prioritising it before submission, not treating it as optional polish |
| **UST maintenance price is a derived proxy and is not PSA-sampled (peer review 2026-08-05, M7)** | UST's current maintenance price (§4.2) is derived, not directly observed (no Part B ASP payment limit or NADAC biosimilar NDC exists for it yet), and the peer review found it carries outsized leverage on the EJP — the paper's headline Aim 1 output — yet `R/06_psa.R` holds it fixed rather than sampling it (unlike Treg's own acquisition price, which IS Gamma-sampled) | **Currently a real gap, deliberately deferred 2026-08-06.** The price's *derivation* is already disclosed (§4.2); what is not yet disclosed anywhere, and should be added regardless of whether the sampling itself gets built, is the fact that it is held fixed despite that outsized leverage. Recommended next step: source a defensible spread (even a wide, explicitly-uncertain one, in the spirit of Decision 4's uniform-prior fallback for π) and add it to `R/06_psa.R`'s sampled-parameter list the same way Treg's own price already is |
| **2-year maintenance cap may not be plausible at either extreme over a lifetime horizon (peer review 2026-08-05, M3)** | §6.4's cap forces ALL remaining UST/IFX/ADA patients onto CT-derived probabilities after 2 years regardless of real-world response; at a 65-year lifetime horizon this means over 96% of the modelled time runs on CT dynamics by construction, while the no-cap alternative (S1) means comparators pay full biologic drug cost for the same 65 years — both are extremes, and no sourced real-world persistence/discontinuation curve exists to locate a defensible middle ground | **Already partly mitigated, not fully resolved.** S1 (§10.3/§10.4) reports both cap-on and cap-off side by side, so the sensitivity is quantified and available, not hidden — the residual gap is narrative: whichever setting is presented as "the" base case should be explicitly caveated in Discussion as a modelling convenience inherited from Aliyev's own 6-year design, not a persistence estimate for a lifetime horizon. Recommended next step, if pursued: replace the binary cap with a sourced time-to-discontinuation curve (§6.4's own original recommendation), which is real, sourceable work, not a data gap this project has already searched and failed to find |
| **No CD-specific excess mortality (peer review 2026-08-05, M6)** | §7.1 item 7 assumes background mortality only (US life tables), no elevated mortality from chronic active Crohn's disease itself — a convention borrowed from Aliyev/NICE TA456, not invented here, but more consequential now that the horizon is 65 years rather than 6 | **Deliberately deferred 2026-08-06 — lowest-risk of the four items in this deferral to leave as-is.** The omission is directionally *conservative with respect to Treg*, not favourable to it: it credits comparator-track patients who spend more cumulative time in active disease with the same mortality as a durably cured Treg patient, which understates rather than inflates Treg's relative survival/QALY advantage. If challenged, this is the most comfortable of the four to defend as-is (precedented, and biased against the technology being evaluated) — recommended next step, if ever pursued, is sourcing a CD-specific standardised mortality ratio to replace the flat assumption, not urgent |
...
**On the four rows above marked "deliberately deferred 2026-08-06":** all four were raised by the same 2026-08-05 external peer review that found B1/B2/B3 (`docs/model_audit_v6.md` A17/A19), and Eric explicitly confirmed the same day that none of them are required for the study as currently scoped — they are possible improvements, not defects in what has already been built, and are kept here as a standing next-steps list rather than being implemented without a clear go-ahead. Re-derive this framing fresh from this table (and from `README.md`'s own matching entry) before assuming any one of them has since been picked up.
